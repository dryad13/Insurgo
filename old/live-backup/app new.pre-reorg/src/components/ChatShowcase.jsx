...
export default function ChatShowcase() {
  ...
  async function handleContactViaWhatsApp() {
    const to = WHATSAPP_CONTACT_NUMBER;
    ...
    const text =
      ...
      'Thanks for contacting Insurgo. We will follow up with a recommended system architecture shortly.';
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setContactWaNote(data?.error || 'WhatsApp send failed. Try again.');
        return;
      }
      setContactWaNote('Message sent on WhatsApp. Check your phone.');
    } catch (err) {
      setContactWaNote(`WhatsApp send failed: ${String(err?.message || err)}`);
    } finally {
