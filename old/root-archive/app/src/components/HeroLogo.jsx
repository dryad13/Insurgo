...
export default function HeroLogo() {
  ...
  /* Track this section's scroll position */
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ['start start', 'end start'],
  });

  /* Parallax: content moves up slightly as you scroll past */
  const heroY = useTransform(scrollYProgress, [0, 1], [0, -80]);
  const heroOpacity = useTransform(scrollYProgress, [0, 0.6, 1], [1, 1, 0]);

  /* Flow steps: each step lights up as you scroll into the hero zone */
  const flowSteps = ['Lead', 'AI Response', 'Qualify', 'Book', 'Sync'];
    ...
    <SectionWrapper className="pt-32 md:pt-40 pb-10 md:pb-16">
      <div ref={sectionRef}>
        ...
        >
          ...
          {/* Animated flow preview — each step staggers in */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
